...
$(CMX): $(BUILD_DIR)/%.cmx: %.ml | $(BUILD_DIR)
	$(OCAMLFIND) $(OCAMLOPT) $(OCAML_COMPILE_FLAGS) $(OCAMLOPT_FLAGS) $(OCAMLFIND_PACKAGES) -o $@ -c $<
$(CMO): $(BUILD_DIR)/%.cmo: %.ml | $(BUILD_DIR)
	$(OCAMLFIND) $(OCAMLC) $(OCAML_COMPILE_FLAGS) $(OCAMLC_FLAGS) $(OCAMLFIND_PACKAGES) -o $@ -c $<
$(BUILD_DIR):
	mkdir $@
